<template>
  <a-layout-sider v-model:collapsed="sideCollapsed"
    :trigger="null"
    :style="{ background: '#fff', overflow: 'auto', height: '100vh' }">
    <div class="leftSider"
      :style="{ width: sideCollapsed ? '80px' : '200px', position: 'fixed', left: 0, top: 0, bottom: 0 }">
      <div :style="{ backgroundColor: theme == 'dark' ? '#001529' : '#fff' }"
        ...
        <p v-if="!sideCollapsed" :style="{ color: theme == 'dark' ? '#fff' : '#1890ff' }" class="app-name">Ant Design</p>
      </div>
      <a-menu
        v-model:selectedKeys="leftSelectedKey"
        v-model:openKeys="leftOpenKeys"
        mode="inline"
        :theme="theme"
...
const { leftMenu, leftSelectedKey, leftOpenKeys } = storeToRefs(userStore)
